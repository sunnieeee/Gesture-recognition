...
/****读取视频图像****/
	while (1)                                  //始终为真的while循环，读取摄像
	{
		Mat frame;                            //定义frame以存放每一帧的图像 
		Mat element = getStructuringElement(MORPH_RECT, Size(10, 10));   //定义待处理的图像元素的形状、大小（椭圆/圆/矩形），参数由调试确定
		capture >> frame;                      //将视频的每一帧读入

		namedWindow("1",WINDOW_AUTOSIZE);
		imshow("1",frame);                     //显示原视频       
...
/****绘制符合条件的轮廓及中心点****/
	   ...
	   //循环控制绘制轮廓
         ...
         */   
				Scalar color = Scalar(255);                              //定义画笔颜色，可设为随机取色
				drawContours(drawing, g_Contours, i, color, 1, 8, vector<Vec4i>(), 0, Point());    //绘制轮廓
